...
- ✅ الزراير الخضرا (قرار عمر، ١٤ سبتمبر): كلهم `bg-success` بهوفر `hover:brightness-[0.92]` زي `.btn-danger:hover`.
  `#059669` ماكانش أخضر واتساب أصلًا، فمفيش هوية نحافظ عليها ومفيش `--success-dark`.

### ✅ المرحلة ١ خضرت (١٤ سبتمبر)

grep الخطة على `app/` و`components/` و`lib/` = ٠ · النسخة الأوسع = ٠ · الحواف والظلال الهيكلية
