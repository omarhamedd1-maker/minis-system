"use client";

import { useState } from "react";

type Customer = {
  id: string;
  full_name: string | null;
  phone: string | null;
  address: string | null;
};

export function CustomerEdit({
  customer,
  updateAction,
}: {
  customer: Customer;
  updateAction: (formData: FormData) => Promise<void>;
}) {
  const [editing, setEditing] = useState(false);

  if (!editing) {
    return (
      <div className="rounded-card bg-surface p-5 shadow-card">
        <div className="flex items-start justify-between">
          <dl className="grid flex-1 gap-3 sm:grid-cols-2">
            <div className="flex justify-between gap-4">
              <dt className="text-ink-muted">التليفون</dt>
              <dd className="text-ink" dir="ltr">
                {customer.phone ?? "—"}
              </dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="shrink-0 text-ink-muted">العنوان</dt>
              <dd className="text-left text-ink">
                {customer.address ?? "—"}
              </dd>
            </div>
          </dl>
          <button
            type="button"
            onClick={() => setEditing(true)}
            className="ms-4 shrink-0 rounded-control bg-sunken px-3 py-1 text-xs font-medium text-ink-body hover:bg-line"
          >
            تعديل
          </button>
        </div>
      </div>
    );
  }

  return (
    <form
      action={updateAction}
      className="rounded-card bg-warning-soft p-5 shadow-card"
    >
      <input type="hidden" name="customer_id" value={customer.id} />
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="flex flex-col gap-1">
          <label className="text-xs text-ink-muted">الاسم</label>
          <input
            name="full_name"
            defaultValue={customer.full_name ?? ""}
            required
            className="rounded-control border border-line-strong px-3 py-1.5 text-sm text-ink focus:border-primary focus:outline-none"
          />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-xs text-ink-muted">التليفون</label>
          <input
            name="phone"
            defaultValue={customer.phone ?? ""}
            dir="ltr"
            className="rounded-control border border-line-strong px-3 py-1.5 text-sm text-ink focus:border-primary focus:outline-none"
          />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-xs text-ink-muted">العنوان</label>
          <input
            name="address"
            defaultValue={customer.address ?? ""}
            className="rounded-control border border-line-strong px-3 py-1.5 text-sm text-ink focus:border-primary focus:outline-none"
          />
        </div>
      </div>
      <div className="mt-3 flex items-center gap-2">
        <button
          type="submit"
          className="rounded-control bg-primary px-4 py-1.5 text-sm font-medium text-white hover:bg-primary-dark"
        >
          حفظ
        </button>
        <button
          type="button"
          onClick={() => setEditing(false)}
          className="rounded-control bg-sunken px-4 py-1.5 text-sm font-medium text-ink-body hover:bg-line"
        >
          إلغاء
        </button>
      </div>
    </form>
  );
}
