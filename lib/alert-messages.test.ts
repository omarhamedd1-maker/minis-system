...
describe("رسالة استيراد شوبيفاي الواقف", () => {
  ...
  /**
   * ⚠️⚠️ **كانت بتقول «اربط المتجر تاني» — والنصيحة دي كانت بتلفّ عمر في
   * دايرة.** التوكن بيموت كل ٢٤ ساعة (`lib/shopify/token.ts`)، فكان يربط،
   * تشتغل يوم، وتقف تاني، والرسالة تقوله يربط تاني.
   *
   * السبب اتصلّح، والرسالة بقت **بتقول اللي حصل ونتيجته** من غير ما تقول
   * اعمل إيه — قرار عمر في كل التنبيهات.
   */
  it("⚠️ بتقول السبب والنتيجة من غير ما تقول اعمل إيه", () => {
    const msg = shopifyImportFailMessage("اتقطع النت");
    const lines = msg.split("\n");
    expect(lines[0]).toContain("استيراد أوردرات شوبيفاي وقف");
    expect(lines[1]).toBe("اتقطع النت");
    expect(lines[lines.length - 1]).toContain("واقفة عند شوبيفاي");
    expect(msg).not.toContain("اربط");
    expect(msg).not.toContain("افتح");
  });
});
