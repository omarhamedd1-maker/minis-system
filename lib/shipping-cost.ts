...
export type ShippingCostInput = {
  ...
  returned?: boolean;
  /**
   * بوسطة حصّلت الفلوس فعلاً؟
   *
   * ⚠️ **لازمتها إن `cod: 0` عند بوسطة معناها حاجتين مختلفتين تمامًا**:
   * يا إما الشحنة أصلاً مالهاش تحصيل، يا إما الفلوس **اتحصّلت خلاص**
   * وبوسطة صفّرت الخانة. اتأكدنا: أوردر ١٣٢٠ (١٬٢٩٨ + ٩٠ شحن، دفع كاش)
   * اتسلّم واتحصّل، وبوسطة بترجّع `cod: 0`.
   *
   * فمن غير الخانة دي، "التحصيل صفر" كان هيشيل رسم التحويل من **كل** أوردر
   * اتسلّم — يعني تكاليفنا تبان أقل من الحقيقة والأرباح تبان أكبر.
   */
  collected?: boolean;
};
...
export function shippingCost(
  ...
  //
  // **والشحنة اللي مالهاش تحصيل أصلاً زيه بالظبط.** الحد الأدنى للتحويل
  // (١٣) بيتطبّق على فلوس بتتحوّل فعلاً. اتأكدنا من شاشة بوسطة: شحنة
  // بتحصيل صفر وقيمة بضاعة ٥٠٠٠ رسومها ٦٤٫٩٨، و٦٤٫٩٨ ÷ ١٫١٤ = ٥٧ بالظبط
  // = ٥٠ تأمين + ٧ رسم فتح. مفيش تحويل.
  //
  // **بس `cod === 0` لوحدها مش كفاية للحكم** — بوسطة بتصفّر الخانة بعد ما
  // تحصّل، فالأوردر المتسلّم بيرجع بصفر برضه. لازم نتأكد إنها ماتحصّلتش.
  const noMoney = returned || (cod === 0 && !input.collected);
