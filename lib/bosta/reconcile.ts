...
export function decideSync(
  ...
  // ⚠️⚠️ **بوسطة بتصفّر التحصيل بعد التسوية — مانمسحش رقمنا بصفرها.**
  //
  // اتأكد من سجل بوسطة نفسها (١٧ أغسطس ٢٠٢٦): أوردر ١٤١٩ اتعمل بتحصيل
  // **٢٠٣٧** الساعة `14:03:42` — نفس ثانية ما سيستمنا عمل الشحنة —
  // و`cod: {before: null, after: 2037}`. والفلوس اتحصّلت فعلًا.
  //
  // ومع كده رد بوسطة النهاردة بيقول `cod: 0`. فالمزامنة كانت بتمسح
  // الـ٢٠٣٧ وتحط صفر، **وشاشات الفلوس تعدّ ناقص**. طلع ٢٧ أوردر كده
  // بإجمالي ٦٣٬٦٩٧ ج — كلهم متحصّلين، وسجلاتنا هي اللي اتمسحت.
  //
  // القاعدة: الصفر الجاي من بوسطة **مايمسحش رقم عندنا أكبر من صفر**.
  // الشحنة الميتة استثناء — دي فعلًا مش هتحصّل حاجة، والصفر بتاعها صح.
  const storedCod = Number(o.bosta_cod ?? 0);
