...
// ==========================================================================
// رابط الويب هوك جوّه الشحنة
// --------------------------------------------------------------------------
// ⚠️ **ده بيلغي أهم خطوة يدوية في تركيب أي عميل**: من غيره لازم صاحب المتجر
// يفتح لوحة بوسطة وينسخ رابط ويلزقه بإيده، وبوسطة **مافيهاش مسار API**
// لتسجيل ويب هوك على مستوى الحساب (اتفحص `GET` و`POST` ١٨ أغسطس ٢٠٢٦).
//
// وتوثيقهم بيقول إن طلب إنشاء الشحنة بيقبل `webhookUrl`، فكل شحنة بتشيل
// رابطها معاها.
// ==========================================================================

describe("رابط الويب هوك في الشحنة", () => {
  const url = "https://minis-system.vercel.app/api/bosta/webhook?key=abc";

  it("بيتبعت لما يكون موجود", () => {
    expect(built({ webhookUrl: url }).base.webhookUrl).toBe(url);
  });

  it("**مابيتبعتش فاضي** — بوسطة ماتخزّنش رابط فاضي وتفضل ترنّ عليه", () => {
    for (const v of [null, undefined, ""]) {
      expect("webhookUrl" in built({ webhookUrl: v }).base).toBe(false);
    }
  });
});
