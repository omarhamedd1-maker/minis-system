...
/**
 * السبب لوحده من غير التواريخ وعدد المحاولات — للإشعار.
 *
 * النص الكامل («العميل رفض يستلم (٣ أغسطس) — ٢ محاولات — اتجدولت ٥ أغسطس»)
 * بياخد سطرين على شاشة القفل، والآيفون بيعرض **٤ سطور بس** — فالسطر اللي
 * بيقول "اعمل إيه" كان بيتقص. التفاصيل موجودة كاملة في شاشة الأوردر.
 */
export function shortReason(text: string | null | undefined): string | null {
  const s = String(text ?? "").trim();
  if (!s) return null;
...
const DEFAULT_ADVICE: ExceptionAdvice = {
  ...
  hint: "كلّم العميل واطلب محاولة تانية، أو رجّع الشحنة",
...
const ADVICE: [RegExp, ExceptionAdvice][] = [
  [
    /بره نطاق|بره التغطية|خارج التغطية/,
    {
      ...
      hint: "اتفق على عنوان جوّه التغطية أو ألغِ الأوردر",
  ...
  [
    ...
    {
      title: "العنوان غلط",
      hint: "صحّح العنوان وقول لبوسطة تحاول تاني",
  ...
  [
    /رقم التليفون غلط|التليفون غلط|رقم غلط/,
    {
      ...
      hint: "صحّح الرقم وقول لبوسطة تحاول تاني",
  ...
  [
    /مش بيرد|مش موجود/,
    {
      ...
      hint: "كلّمه واتفق على معاد يكون فيه موجود",
      actions: ["whatsapp", "cancel"],
    },
  ],
  [
    /رفض يستلم|رفض الاستلام/,
    {
      ...
      hint: "كلّمه — ولو مصمم رجّع الشحنة وألغِ الأوردر",
      actions: ["whatsapp", "cancel"],
    },
  ],
  [
    /مش معاه فلوس|مافيش فلوس/,
    {
      ...
      hint: "اتفق على معاد يكون جهّز فيه المبلغ",
      actions: ["whatsapp", "cancel"],
    },
  ],
  [
    /طلب التأجيل|تأجيل/,
    {
      ...
      hint: "اتجدولت؟ استنى المعاد. مش متأكد؟ كلّمه",
      actions: ["whatsapp"],
    },
  ],
  [
    /اتلفت|ضاعت/,
    {
      ...
      hint: "كلّم بوسطة على التعويض وقرر الأوردر",
      actions: ["whatsapp", "cancel"],
    },
  ],
  [
    /اتلغت|ملغية/,
    {
      ...
      hint: "اعمل شحنة جديدة أو ألغِ الأوردر",
      actions: ["cancel"],
    },
  ],
];
